...
const radiusLimit = 2;
const colorsArray = ["#ffffff", "red", "gold"];

export const ImpactCloud = () => {
  const ref = useRef<THREE.Points>(null);

  //   const [fall, setFall] = useState(0.1);

  useFrame(() => {
    const positions = (
      ref.current?.geometry.getAttribute("position") as THREE.BufferAttribute
    ).array as Float32Array;
    const velocities = (
      ref.current?.geometry.getAttribute("velocity") as THREE.BufferAttribute
    ).array as Float32Array;
  ...
  return (
    <points ref={ref} frustumCulled={false}>
      ...
      <pointsMaterial opacity={0.8} size={0.2} transparent vertexColors />
    </points>
  );
};
